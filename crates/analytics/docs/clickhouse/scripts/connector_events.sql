...
-- ---------------------------------------------------------------------------
-- Unified Connector Service (UCS) connector-call events.
--
-- UCS emits the same connector-call audit event from its own Kafka topic, in
-- its native field shape. These views map that shape onto the shared
-- `connector_events` tables and stamp `source = 'unified_connector_service'`
-- (`destination = 'connector'`, since a UCS connector-call always targets the
-- connector). Field mapping: reference_id -> payment_id, resource_id ->
-- refund_id, lineage_merchant_id -> merchant_id, flow_type -> flow,
-- connector -> connector_name, request_data -> request,
-- response_data -> masked_response, timestamp(ms) -> created_at,
-- latency_ms -> latency.
-- ---------------------------------------------------------------------------

CREATE TABLE ucs_connector_events_queue
(
    `request_id` String,
    `timestamp` Int64,
...
CREATE MATERIALIZED VIEW ucs_connector_events_parse_errors (
    `topic` String,
    `partition` Int64,
    `offset` Int64,
    `raw` String,
...
SELECT
    ...
    _error AS error
FROM
    ucs_connector_events_queue
WHERE
    length(_error) > 0;

CREATE MATERIALIZED VIEW ucs_connector_events_mv TO connector_events (
    `merchant_id` String,
    `payment_id` Nullable(String),
    `connector_name` LowCardinality(String),
    `request_id` String,
...
CREATE MATERIALIZED VIEW ucs_connector_events_audit_mv TO connector_events_audit (
    `merchant_id` String,
    `payment_id` String,
    `connector_name` LowCardinality(String),
    `request_id` String,
